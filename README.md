# 👋 Hi, I'm Rahul Dutt

**11th Grade @ Levine Middle College High School | Charlotte, NC**
Dual-enrolled at NC School of Science & Math + Central Piedmont Community College

[![Email](https://img.shields.io/badge/Email-1rahuldutt%40gmail.com-blue?style=flat&logo=gmail)](mailto:1rahuldutt@gmail.com)
[![USACO Bronze](https://img.shields.io/badge/USACO-Bronze%20(100%25)-gold?style=flat)](https://usaco.org)
[![Hackathon](https://img.shields.io/badge/UNC%20Charlotte%20Hackathon-1st%20Place%202025-brightgreen?style=flat)](https://charlotte.edu)

---

## 🧠 About Me

I'm a student specializing in **Computer Science**, **Applied AI**, and **Machine Learning**. I love building AI-powered tools, competing in programming and math competitions, and teaching others through community initiatives.

---

## 🚀 Projects

| Project | Description | Stack |
|---|---|---|
| [AI Collaborative Browser](./project-ai-browser.html) | AI-powered browser with group research & URL scoring | PyQt5, Gemini API |
| [DNA Sequence Detection](./project-dna.html) | Detects start/stop codons in DNA strands using AI | Python, TensorFlow, NumPy |
| [Peptide Interaction Simulation](./project-peptide.html) | Simulates molecular interactions via thermodynamic values | NumPy, Matplotlib |
| [Diabetic Retinopathy Detection](./project-retinopathy.html) | CNN model detecting DR via retinal blood vessel annotation | TensorFlow, NumPy, Pandas |
| [Breast Cancer CNN](./project-cancer.html) | Image-based breast cancer prediction (88% accuracy) | Python, TensorFlow |
| [Image Super Resolution](./project-image-sr.html) | Facial image sharpening via CNN + Django web UI | Python, Django, TensorFlow |
| [Cereal Milk Initiative](./project-cereal-milk.html) | STEM community learning program for K-8 students | Education / Outreach |

---

## 🏆 Awards & Highlights

- 🥇 **1st Place** — UNC Charlotte Annual Hackathon (2025)
- 🥇 **1st Place** — College of Charleston Programming Competition (2023)
- 🏅 **USACO Bronze** — Cleared with 100% score (2023)
- 📐 **AIME Qualifier** — via AMC 10 (2021, 2022, 2023)
- 🔬 **4th Place Regional** — Science Fair, Technology category (2024)
- 🌟 **AMC 8 Distinguished Honor Roll** (2022, 2023)
- 🎓 **Founder** — Technology Student Association Club & Cereal Milk Initiative

---

## 🛠️ Tech Stack

**Languages:** Python · Java · JavaScript · C (Arduino) · C# · R · HTML  
**Frameworks & Tools:** TensorFlow · Django · PyQt5 · NumPy · Pandas · Matplotlib · Unity · Android Studio  
**AI/ML:** CNNs · ANNs · Logistic Regression · Linear Regression · Gemini LLMs · Image Super-Resolution  
**Platforms:** Google Colab · Visual Studio · Spyder · MIT App Inventor

---

## 📂 Repository Structure

```
rahul-portfolio/
├── README.md                        ← You are here
├── SETUP.md                         ← How to run any project locally
├── projects/
│   ├── ai-browser/
│   ├── dna-sequence-detection/
│   ├── peptide-simulation/
│   ├── diabetic-retinopathy/
│   ├── breast-cancer-cnn/
│   ├── image-super-resolution/
│   └── cereal-milk-initiative/
└── .github/
    └── CONTRIBUTING.md
```

---

## 📬 Contact

- 📧 Email: [1rahuldutt@gmail.com](mailto:1rahuldutt@gmail.com)
- 📍 Charlotte, NC
- 📞 (980) 552-1422
